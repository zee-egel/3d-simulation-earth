...
import {
  ...
  VEHICLES,
  lanePoint,
} from "./trafficSimulation.ts";
import type { TrafficConfig, Point } from "./trafficSimulation.ts";

const PALETTE = [
  0xe65e43, 0x57b8b2, 0x4a83cb, 0xf1bd4b, 0xe8e5de, 0x7757a0, 0x8fab57,
  0x374451,
];
...
export function createTraffic(
  ...
  const signals = batch(
    scene,
    simulation.junctions.length * 4 * 3 + simulation.crossings.length * 4,
  );
  const furniture = batch(
    scene,
    simulation.junctions.length * 4 * 7 + simulation.crossings.length * 12,
  );
  ...
  for (const crossing of simulation.crossings) {
    for (const side of [-1, 1]) {
      const pedAngle = crossing.horizontal
        ? (-side * Math.PI) / 2
        : side < 0
          ? 0
          : Math.PI;
      const p = crossing.horizontal
        ? { x: crossing.x + side * 1.95, z: crossing.z + 0.25 }
  ...
  for (const j of simulation.junctions) {
    ...
    for (let d = 0; d < 4; d++) {
      if (j.neighbors[(d + 2) % 4] < 0) continue;
      const p = lanePoint(j, d, -STOP_LINE),
        angle = Math.atan2(DIRECTIONS[d].x, DIRECTIONS[d].z);
  ...
  for (const c of simulation.crossings) {
    ...
    for (const side of [-1, 1]) {
      const pedAngle = c.horizontal
        ? (-side * Math.PI) / 2
        : side < 0
          ? 0
          : Math.PI;
      const p = c.horizontal
        ? { x: c.x + side * 1.95, z: c.z + 0.25 }
        : { x: c.x + 0.25, z: c.z + side * 1.95 };
      bulbs.push({
        index: signalIndex,
        junction: c.junction,
        direction: 4,
  ...
  panel.open = true;
  panel.style.cssText =
    "position:fixed;bottom:12px;left:12px;z-index:1001;background:#101923ed;color:#eef3f7;padding:12px;border:1px solid #ffffff26;border-radius:10px;font:13px/1.5 system-ui;max-width:280px";
  panel.innerHTML = `<summary style="cursor:pointer;font-weight:650">Control environment</summary>
  ...
  for (const input of [carInput, peopleInput]) {
    ...
    input.addEventListener("change", () =>
      simulation.setCounts(Number(carInput.value), Number(peopleInput.value)),
    );
  }
  let paused = false;
  panel.querySelector("#pause")!.addEventListener("click", () => {
    paused = !paused;
    panel.querySelector("#pause")!.textContent = paused
      ? "Resume traffic"
      : "Pause traffic";
  });
  let visitIndex = 0;
  const destinations = simulation.crossings
    .slice()
  ...
  function update(delta: number) {
    ...
    let index = 0,
      lightIndex = 0;
    for (const car of driving.car
      ? [...simulation.cars, driving.car]
      : simulation.cars) {
    ...
    for (const person of simulation.people) {
      ...
      const h = person.height,
        base = person.state === "cross" ? 0.12 : 0.2;
      const swing =
        person.state === "wait" ? 0 : Math.sin(person.walkTime * 8) * 0.5;
      box(
        people,
        index++,
        p,
      ...
      box(
        ...
        h * 0.23,
        h * 0.2,
        [0xe7b68e, 0xa96f4c, 0x72482f][person.id % 3],
      );
      for (const side of [-1, 1]) {
        box(
          people,
    ...
    for (const bulb of bulbs) {
      ...
      const state =
        bulb.direction === 4
          ...
          : controller.vehicle(bulb.direction);
      const on =
        bulb.aspect === 0
          ? state === "red"
          : bulb.aspect === 1
            ? state === "amber"
            : state === "green" ||
              (state === "flash" && simulation.time % 1.2 < 0.6);
      if (on !== bulb.last) {
    ...
    if (statusTime >= 0.5) {
      statusTime = 0;
      panel.querySelector("#traffic-status")!.textContent =
        `${simulation.completedTrips} junction passages · ${simulation.people.filter((p) => p.state === "wait").length} waiting to cross`;
    }
  }
