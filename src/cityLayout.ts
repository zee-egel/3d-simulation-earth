...
export function shoreRadius(angle: number, phase: number) {
  return (
    1 +
    0.085 * Math.sin(angle * 3 + phase) +
    0.045 * Math.sin(angle * 5 - phase)
  );
}
export function landscapeDistance(x: number, z: number, feature: Landscape) {
  const nx = (x - feature.x) / feature.rx,
    nz = (z - feature.z) / feature.rz;
...
export function createCityLayout(
  ...
  const features: Landscape[] = [
    ...
    {
      name: "Weteringpark (zou ik niet komen)",
      x: 0.34,
      z: -0.12,
      rx: 0.085,
      rz: 0.14,
  ...
  function densityAt(x: number, z: number) {
    const hubs = Math.max(
      ...centers.map(
        (c) =>
          c.weight *
          Math.exp(-((x - c.x) ** 2 + (z - c.z) ** 2) / c.radius ** 2),
      ),
    );
    const texture =
      0.11 * Math.sin(x / 22 + phase) * Math.cos(z / 29 - phase) +
      0.07 * Math.sin((x + z) / 13);
    return Math.max(0, Math.min(1, hubs + texture + 0.12));
  }
  function districtAt(x: number, z: number): District {
    const value = densityAt(x, z);
    return value > 0.7 ? "downtown" : value > 0.29 ? "urban" : "suburban";
  }
  function reserved(x: number, z: number, margin = 0) {
    return features.some(
      (f) => landscapeDistance(x, z, f) < 1 + margin / Math.min(f.rx, f.rz),
    );
  }
  const n = citySize - 1;
  ...
  for (let x = 0; x < n; x++)
    for (let z = 0; z < n; z++)
      nodes.push({
        x: x * spacing - offset + spacing / 2,
        z: z * spacing - offset + spacing / 2,
        neighbors: [-1, -1, -1, -1],
      });
  for (let x = 0; x < n; x++)
    for (let z = 0; z < n; z++) {
      ...
      for (const [direction, to] of [
        ...
        if (
          [0, 0.25, 0.5, 0.75, 1].some((t) =>
            reserved(a.x + (b.x - a.x) * t, a.z + (b.z - a.z) * t, 5),
          )
        )
          continue;
        ...
        const keep =
          avenue ||
          random() < (districtAt(a.x, a.z) === "downtown" ? 0.9 : 0.58);
        if (keep) {
          a.neighbors[direction] = to;
          b.neighbors[(direction + 2) % 4] = id;
        }
      }
    }
  ...
  while (changed) {
    ...
    nodes.forEach((node) => {
      ...
      node.neighbors.forEach((to, d) => {
        if (to >= 0) {
          nodes[to].neighbors[(d + 2) % 4] = -1;
          node.neighbors[d] = -1;
          changed = true;
        }
      });
  ...
  nodes.forEach((node, id) => {
    ...
    for (let i = 0; i < component.length; i++)
      for (const to of nodes[component[i]].neighbors) {
        if (to >= 0 && !visited.has(to)) {
          visited.add(to);
          component.push(to);
        }
      }
  ...
  nodes.forEach((node, id) => {
    if (!retained.has(id)) node.neighbors.fill(-1);
  });
  function hasStreet(blockX: number, blockZ: number) {
    for (const x of [blockX - 1, blockX])
      for (const z of [blockZ - 1, blockZ]) {
        if (x >= 0 && z >= 0 && x < n && z < n && retained.has(x * n + z))
          return true;
      }
    return false;
  }
